...
function    getImgComments($img_id) {
    $get_gallery = db_connect()->prepare("SELECT user.user_name, comments.comment_txt FROM comments INNER JOIN user ON user.user_id=comments.comment_user WHERE comments.comment_img=:img_id ORDER BY comment_time");
    $get_gallery->bindParam(':img_id', $img_id);
    $get_gallery->execute();
    return ($get_gallery->fetchAll());
}

function    getImgLikes($img_id) {
    $get_gallery = db_connect()->prepare("SELECT COUNT * FROM likes WHERE `like_img`=:img_id");
    $get_gallery->bindParam(':img_id', $img_id);
    $get_gallery->execute();
    return ($get_gallery->fetchAll());
}
