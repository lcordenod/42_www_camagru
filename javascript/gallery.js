...
function generateSingleUserGallery(
  img_path,
  comments,
  likes,
  username,
  ...
  if (comments.length != undefined)
    icons.innerHTML =
      ...
      likes +
      '</span><span class="social-like-icon">👍</span> <span class="comment-count">' +
      comments.length +
      "</span> 💬</div>";
  else
    icons.innerHTML =
  ...
  comment_length_error.innerHTML =
    "Comment is too long, must be less than 150 characters";
  ...
  } else gallery.appendChild(picture);
...
function displayMoreImagesBtn() {
  ...
  document
    ...
    .appendChild(more_images_btn);
}

function removeMoreImagesBtn() {
  document
    .getElementById("more-images-btn")
    .parentNode.removeChild(document.getElementById("more-images-btn"));
}
  ...
  xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      if (this.responseText.includes("Catched exception"))
        ...
        document.getElementById("my-gallery-pictures-count").innerText =
          gallery_array[0][0];
        checkInput(gallery_array[0][5]);
        if (
          gallery_array[0][0] - offset > 5 &&
          !document.getElementById("more-images-btn")
        )
  ...
  xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      if (this.responseText.includes("Catched exception"))
        ...
        document.getElementById("my-gallery-feed").innerHTML =
          '<span style="display:block;text-align:center;">No images yet :(</span>';
    }
  };
...
function getImageView(img_id) {
  ...
  xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      } else alert("An error happened when loading the image");
    }
  };
...
function deleteImage(img_id) {
  ...
  var url_img = url.match(/\?id=([0-9]+)/)[1];
  if (img_id == url_img) {
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        if (this.responseText == "Image deleted") {
          window.history.back();
          alert("Image deleted");
        } else alert("An error happened when deleting the image");
      }
    };
    ...
    alert("You cannot delete image " + img_id);
    return false;
  }
}

window.addEventListener("load", function(e) {
  if (window.location.href.indexOf("index.php") != -1) getAllUsersGallery(0);
