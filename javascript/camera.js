...
function resetCamera() {
  ...
  if (document.getElementsByClassName("pictures-taken-picture").length === 0) {
    ...
    document.getElementById(
      "message-no-pictures-taken-box"
    ).style.backgroundColor = "rgba(78, 167, 255, 0.4)";
  }
}
...
function takePicture() {
  ...
  if (width) {
    if (is_image_upload === false) {
      ...
      var data = canvas.toDataURL("image/png");
    } else var data = upload_src;
    picture.setAttribute("src", data);

    if (
      document.getElementsByClassName("pictures-taken-picture").length === 0
    ) {
      document.getElementById("message-no-pictures-taken-box").innerHTML =
        "Looking good! 😎<br/>Now you can save it or retry";
      document.getElementById(
        "message-no-pictures-taken-box"
      ).style.backgroundColor = "rgba(59, 255, 59, 0.4)";
    }
...
function showMontageInPicturesTakenBox() {
  ...
  var check_existing_picture = document.getElementsByClassName(
    "pictures-taken-picture"
  );
  ...
  if (check_existing_picture === null)
    ...
    pictures_taken_box.insertBefore(
      new_montage,
      pictures_taken_box.childNodes[0]
    );
}
...
function showCameraBooth() {
  camera_container = document.getElementsByClassName("camera-container")[0];
  title_filters = document.getElementById("title-filters");
  filter_selector = document.getElementById("filter-selector");
  camera_container.style.display = "block";
  title_filters.style.visibility = "visible";
  filter_selector.style.visibility = "visible";
}
...
(function() {
  ...
  function startCamera() {
    ...
    setTimeout(showCameraBooth, 1250);
  ...
  save_button.addEventListener(
    ...
    function(e) {
      ...
      document.getElementById("message-no-pictures-taken-box").style.display =
        "none";
      e.preventDefault();
    },
    false
